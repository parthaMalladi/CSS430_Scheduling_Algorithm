...
// Function to compare two tasks by priority, and lexicographically if priorities are equal
static bool comesBefore(Task *a, Task *b) {
    // First, compare priorities (higher priority comes first)
    if (a->priority > b->priority) {
        return true;
    } else if (a->priority == b->priority) {
        // If priorities are equal, compare names lexicographically
